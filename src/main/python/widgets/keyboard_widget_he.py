...
class KeyWidget:
    ...
    def setState(self, state: int):
        """Set the visual state (0=normal, 1=rt, 2=crt)."""
        self.state = state
...
class KeyboardWidgetHE(QWidget):
    ...
    def __init__(self, layout_editor):
        ...
        self.input_priority_index = 0
        self.input_priority_pair = [None, None]
    ...
    def paintEvent(self, event):
        ...
        mask_rt_brush = QBrush()
        mask_rt_brush.setColor(QColor("#1A9FFF"))
        mask_rt_brush.setStyle(Qt.SolidPattern)

        mask_crt_brush = QBrush()
        mask_crt_brush.setColor(QColor("#1A9FFF").darker(140))
        mask_crt_brush.setStyle(Qt.SolidPattern)
        ...
        # for rt/crt
        background_rt_brush = QBrush()
        background_rt_brush.setColor(QColor("#1A9FFF").darker(120))
        background_rt_brush.setStyle(Qt.SolidPattern)

        foreground_rt_brush = QBrush()
        foreground_rt_brush.setColor(QColor("#1A9FFF"))
        foreground_rt_brush.setStyle(Qt.SolidPattern)

        background_crt_brush = QBrush()
        background_crt_brush.setColor(QColor("#1A9FFF").darker(170))
        background_crt_brush.setStyle(Qt.SolidPattern)

        foreground_crt_brush = QBrush()
        foreground_crt_brush.setColor(QColor("#1A9FFF").darker(140))
        foreground_crt_brush.setStyle(Qt.SolidPattern)

        mask_font = qp.font()
        # mask_font.setPointSize(round(mask_font.pointSize() * 0.8))
        mask_font.setPointSize(round(mask_font.pointSize()))
        ...
        for idx, key in enumerate(self.widgets):
            ...
            active = key.active or (key in self.selected_keys) or (key in self.input_priority_pair)
            ...
            if key.state == 0:
                ...
                qp.setBrush(background_rt_brush)
            elif key.state == 2:
                qp.setBrush(background_crt_brush)
            ...
            if key.state == 0:
                ...
                qp.setBrush(foreground_rt_brush)
            elif key.state == 2:
                qp.setBrush(foreground_crt_brush)
            ...
            if key.masked:
                ...
                if key.state == 0:
                    ...
                    qp.setBrush(mask_rt_brush)
                elif key.state == 2:
                    qp.setBrush(mask_crt_brush)
    ...
    def enable_interaction(self):
        """Allow user to interact with the keyboard."""
        self.enabled = True
        self.setCursor(Qt.ArrowCursor)
        self.setFocusPolicy(Qt.StrongFocus)

    def disable_interaction(self):
        """Prevent user from interacting (clicks, hovers, etc)."""
        self.enabled = False
    ...
    def mousePressEvent(self, ev):
        ...
        if self.tab_index != 3:
            if key is not None:
                ...
                if ctrl:
                    ...
                    # No Ctrl: single-selection logic
                ...
                self.last_key = self.selected_keys[0] if self.selected_keys else key
                self.clicked.emit()
            else:
                self.selected_keys.clear()
                self.deselected.emit()
        ...
        existing_key = any(
            key in (key1, key2) 
            for layer, key1, key2, resolution in self.input_priority_pairs 
        ...
        if key is not None and self.select_enabled and not existing_key and same_side:
            ...
            if key in pair:
                # Remove key from pair
                remove_idx = pair.index(key)
                pair[remove_idx] = None       # clear it
                self.input_priority_index = remove_idx


            elif idx is not None and pair[idx] is None:
                # Assign key to the empty slot
                pair[idx] = key

                # Find open slots
                empty_slots = [i for i, v in enumerate(pair) if v is None]

                if len(empty_slots) == 1:
                    self.input_priority_index = empty_slots[0]
                else:
                    self.input_priority_index = None
    ...
    def leaveEvent(self, ev):
        if self.tab_index == 3:
            if hasattr(self, "_hovered_keys"):
                for key in self._hovered_keys:
                    key.setText("")
                self._hovered_keys.clear()
                self.update()
            super().leaveEvent(ev)

    def select_all(self):
        """Select all keys in the current layout."""
        self.selected_keys = list(self.widgets)
        # self.last_key = self.widgets[0]
        self.update()
        # self.clicked.emit()
